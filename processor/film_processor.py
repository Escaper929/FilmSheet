...
class FilmProcessor:
    ...
    def _render_135(self, images, status_callback, progress_callback):
        ...
        img_idx = 0
        edge_font_sz = int(14 * thumb_w / 400) if thumb_w > 200 else 14
        base_edge_text = self.config.get('edge_text', '')
        ...
        for row in range(rows):
            ...
            perf_y_top = current_y + perf_center_offset_px
            perf_y_bottom = current_y + strip_h - perf_center_offset_px
            y_img_top = current_y + frame_top_offset_px

            # ---- 齿孔 ----
            x = 25
            while x < total_w - 25:
                engine.draw_single_perf(draw, int(x), int(perf_y_top), perf_fill, perf_type)
                engine.draw_single_perf(draw, int(x), int(perf_y_bottom), perf_fill, perf_type)
                x += pitch_px
            ...
            candidate_positions = [
                margin + int(0.2 * (total_w - 2*margin)),
                margin + int(0.5 * (total_w - 2*margin)),
                margin + int(0.8 * (total_w - 2*margin))
            ]
            selected_x = random.sample(candidate_positions, num_occurrences)

            text_y = current_y + engine.mm_to_px(0.1) + edge_font_sz // 2

            for x_pos in selected_x:
                self.draw_edge_text_at(draw, edge_text, x_pos, text_y, edge_font_sz, render_style)
